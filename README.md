...
| 攻撃 | 自動 |
| 特技（御札） | 自動（周期的に放つ） |
| 詠唱 | Z / J（主神の技。珠 2 つ。使うと珠が飛んでいき、拾うと回数が戻る。波を越えても戻る） |
...
5. 神格は位上がりで選んだ神と神酒で上がるほか、神器を敵に当てて**神徳**が溜まっても上がる。
...
- タッチ端末では描画解像度を画面幅の 75%（`Cfg.MOBILE_RENDER_SCALE`）に落として引き伸ばす。論理座標は 640×H のまま。
