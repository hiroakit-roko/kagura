...
2. 敵を倒すと勾玉（経験値）が落ちる。位（レベル）が上がった瞬間に時間が止まり（敵も弾も動かない）、
   神との邂逅になる。
3. **最初の邂逅では 3 柱から主神を選ぶ。** 主神はすぐに稀以上の恩恵を授ける。
4. 以降は 1 柱の神が現れ、3 つの恩恵から 1 つを選ぶ。経験値曲線はおよそ 1 波に 1 回の邂逅になるよう調整している。
...
| 神 | 得意（シューティングとしての軸） | 神威（味付け） | 対応する Hades の神 |
| --- | --- | --- | --- |
| 天照大神 | 貫通光線、敵弾の反射・消去、被ダメ軽減 | 照覧（被ダメ増） | Athena |
| 須佐之男命 | 3 方向拡散、押し戻し、大波、天叢雲の一閃 | 裂傷（移動でダメージ） | Poseidon |
| 建御雷神 | 連鎖する雷、落雷、雷雲 | 帯電（攻撃時に自傷） | Zeus |
| 月読命 | 時限爆発（範囲）、回転する月輪、低 HP 即死 | 宿命 | Ares |
| 天宇受売命 | 敵弾を花弁に変えて消す、魅了（敵の寝返り） | 弱体（与ダメ減） | Aphrodite |
| 宇迦之御魂神（稲荷） | 誘導する狐火、眷属（オプション）、会心 | 狐憑き（次弾会心） | Artemis |
| 少名毘古那神 | 毒霧の領域、回復、常世の妙薬 | 酩酊（重なる継続ダメージ） | Dionysus |
| 伊邪那美命 | 減速、凍結、命中で砕ける氷片、凍土 | 冷気（限界で砕ける） | Demeter |
| 猿田彦大神 | 移動・連射・弾速・疾走・詠唱の加護のみ（主神候補外） | ― | Hermes |
