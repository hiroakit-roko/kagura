...
| 攻撃 | 自動 |
| 特技（御札） | 自動（周期的に放つ） |
| 詠唱 | Z / J（主神の技。札 2 枚。使うと札が飛んでいき、拾うと回数が戻る。波を越えても戻る） |
...
4. **位 4 と位 6 では残る神から 3 柱が現れ、副神を迎える**（神器の威力は主神と同じ。同じ契約の確認あり）。
   主神と副神の差は、詠唱（Z）と神招き（X）が主神のものになる点だけ。
   それ以外の位上がりでは**迎えている神から 1 柱を自分で選ぶ**。その神の神格が 1 上がり、続けてその神の能力 3 枚が抽選される。
   ボスを倒した褒賞は**神宝**（常時効果の宝物。波ごとに HP 回復、最大 HP +30、詠唱の札 +1、使い魔の強化・分身など 17 種）を 3 つから 1 つ選ぶ。
...
5. 神格は位上がりで選んだ神が上がるほか、神器を敵に当てて**神徳**が溜まっても上がる。
   威力は 1 段ごとに +12%。**近距離で使いにくい神器は見返りとして伸びが大きい**（月読・天宇受売 +18%、須佐之男 +15%）。節目で弾数や大きさが増える。
7. 詠唱（Z）と神招き（X）は主神の技。神招きのゲージは時間で溜まる（満タン約 45 秒）。与ダメージ・被弾・かすりは少し足すだけで、神招きの最中は溜まらない。1/4 で発動、満タンで大神招き。

## 恩恵システム（Hades との対応）

| Hades | 本作 |
| --- | --- |
| 神ごとの恩恵で武器が変わる | 神を迎えると**神器**（自動発射の武器）が付く。主神も副神も同じ威力（差は詠唱・神招きだけ） |
...
説明文の「威力 N%」は 自機の基礎攻撃 × その神の神格倍率 に対する割合で、位と神格で伸びる（固定値は後半に埃をかぶるため）。
継続的に画面を覆う神器（日輪光線・月輪）の消弾は常時ではなく、閃き（陽炎）や刃の接触に限っている。
...
- PWA として書き出している（standalone・縦固定）。ホーム画面に追加すると URL バーなしの全画面で起動する。
- ブラウザで開いた場合は最初のタップで全画面と縦固定を要求する（Android Chrome。iOS Safari は全画面 API が無いためホーム画面に追加が必要）。
- タッチ端末で横向きになると止まって「縦にしてください」と出る。
